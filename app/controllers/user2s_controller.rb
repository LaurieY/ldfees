class User2sController < ApplicationController
  ...
  def post_data
    message=""
    user2_params = { :id => params[:id],:pseudo => params[:pseudo],:email => params[:email],:firstname => params[:firstname] }
    case params[:oper]
    when 'add'
      if params["id"] == "_empty"
    ...
    #  end
    redirect_to :controller=>'user2', :action=>'new'
    when 'del'
  ...
  def index
    ...
    @user2s=User.paginate(conditions)
