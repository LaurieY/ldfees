module Jqgrid
  ...
  def jqgrid_stylesheets(theme="default")
    ...
    :cache => "jqgrid/themes/#{theme}/jqgrid-#{theme}-css"  # LEY
#:cache=>true
    end
    ...
    def jqgrid(title, id, action, columns = [], options = {})
      ...
      custom_buttons = ''
      logger.auto_flushing=true  #LEY
      ##logger.add 999, "******going into custom buttons  with #{options[:customButtons].inspect}  of size #{options[:customButtons].size} and type #{options[:customButtons].class}****"  #LEY
      if options[:customButtons].size > 0 && options[:customButtons].is_a?(Array)
   ##     logger.add 999, "****** custom buttons is an array****"  #LEY
   ...
   ##        logger.add 999, "**** Custom Buttons= #{custom_buttons}"  # LEY
