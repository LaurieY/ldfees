...
@logger.debug  "row[hashCols[referenceCol]]  = #{row[hashCols["referenceCol"]]} goes to #{hXref[row[hashCols["referenceCol"]].to_i][1] } "
  ...
  @logger.debug  "ocmEntry = #{ocmEntry} "
...
@logger.debug " Date on this line is #{dateStr} "
...
@logger.info "Debugger Reference column = #{hashC["referenceCol"]}" 
...
@logger.debug "Row with a date is #{arrayC[2]} "
...
def self.findColValue(feesFile,searchFor,arrayC) # New version June 2014
 # **
  ...
  sheetC.each { |row|
 @logger.info "searchFor= #{searchFor} in Row #{row}"
...
#puts "C #{row[col].class}"
     ...
     @logger.info " found decimal value at column #{col} value #{row[col]}\n"
     arrayC[1] = col
